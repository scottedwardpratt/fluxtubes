...
void NS_SU3::ReadNpq(vector<vector<vector<double>>> &Npq,int Amax,vector<vector<double>> &biggestweight){
	...
	for(a=0;a<=Amax;a++){
		...
		if(a>0){
			snprintf(filename,120,"opentrajectories/Npq_A%d.dat",a);
...
void NS_SU3::WriteOpenTrajectories(int A,long long int ntraj,CRandy *randy){
	...
	for(p=0;p<=pmax;p++){
		...
		for(q=0;q<=p;q++){
				snprintf(filename,120,"opentrajectories/A%d/p%d_q%d.dat",A,p,q);
...
void NS_SU3::CalcPQCount(int p0,int q0,int Amax,vector<vector<vector<double>>> &pqcount){
  ...
  pqlist=new Tpqlist *[Amax+1];
	pmax=GetPmax(ngluons)+nquarks+nanti+2*p0+2*q0+1;
  for(A=0;A<=Amax;A++)
		pqlist[A]=new Tpqlist(pmax);
  pqlist[0]->add(p0,q0,1);
...
void NS_SU3::ClearPQCount(int Amax,vector<vector<vector<double>>> &pqcount){
	for(int A=0;A<=Amax;A++){
		for(int p=0;p<pqcount[A].size();p++){
			for(int q=0;q<=pqcount[A].size();q++){
				pqcount[A][p][q]=0;
			}
		}
	}
...
void NS_SU3::CalcCasimirs(int p,int q,double &Cquad,double &Ccubic){
	Cquad=(p*p+q*q+3*p+3*q+p*q)/3.0;
	Ccubic=(p-q)*(3.0+p+2.0*q)*(3.0+q+2.0*p)/18.0;
}
